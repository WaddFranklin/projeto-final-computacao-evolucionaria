...
def leiaInt(msg):
    while True:
        try:
            ...
            print(f'{Color.DANGER}[Erro]: Digite um número inteiro válido.{Color.RESET}')
            continue
        except (KeyboardInterrupt):
            print(f'{Color.DANGER}[Erro]: Usuário preferiu não digitar esse número.{Color.RESET}')
            return 0
        else:
            return n
...
def leiaFloat(msg):
    while True:
        try:
            ...
            print(f'{Color.DANGER}[Erro]: Formatação incorreta. Use ponto no lugar de vírgula.{Color.RESET}')
            continue
        except (KeyboardInterrupt):
            print(f'{Color.DANGER}[Erro]: Usuário preferiu não digitar esse número.{Color.RESET}')
            return 0
